  ...
  return (
    <DivPokemonCard onClick={onClick} key={id} style={{ maxWidth: '320px' }}>
      ...
      <DivSpecies>
        ...
        <DivSpeciesCard>
          <PValue>
            {abilities.charAt(0).toUpperCase() + name.slice(1).toLowerCase()}
          </PValue>
          <PSpecies>Abilities</PSpecies>
        </DivSpeciesCard>
        <DivSpeciesCard>
          <PValue>{weight}</PValue>
          <PSpecies>Weight</PSpecies>
        </DivSpeciesCard>
        <DivSpeciesCard>
          <PValue>{base_experience}</PValue>
          <PSpecies>Base Experience</PSpecies>
        </DivSpeciesCard>
      </DivSpecies>
      <DivButton>
        {isLoggedIn && <ButtonAddArena id={id} arena={arena} />}
        {isLoggedIn && (
          <ButtonFavorite id={id} favorite={favorite}></ButtonFavorite>
        )}
      </DivButton>
      {children}
    </DivPokemonCard>
